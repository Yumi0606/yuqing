  ...
  <div class="management-page">
    ...
    <div class="stats-cards">
      ...
      <div class="stat-card">
        ...
        <div class="stat-value">{{ formatNumber(totalData) }}</div>
    ...
    <div class="content-section">
      ...
      <div class="plans-table-container">
        <table class="plans-table" v-if="plans.length > 0">
          ...
          <tbody>
            <tr v-for="plan in plans" :key="plan.keyid">
              <td>{{ plan.keyword }}</td>
              ...
              <td>{{ plan.alertCount || 0 }}</td>
              <td>{{ formatNumber(plan.dataCount) }}</td>
              <td>
                <span 
                  class="status-badge" 
                  :class="{ active: plan.isEnabled }"
                >
                  {{ plan.isEnabled ? '启用中' : '已停用' }}
                </span>
              </td>
              <td>
                <div class="action-buttons">
                  ...
                  <button @click="togglePlanStatus(plan)" class="btn-action toggle">
                    <i class="fas" :class="plan.isEnabled ? 'fa-pause-circle' : 'fa-play-circle'"></i>
                  </button>
          ...
          <div class="form-group">
            ...
            <div class="toggle-container">
              <label class="toggle">
                ...
                <span class="toggle-label">{{ formData.isEnabled ? '启用' : '停用' }}</span>
              </label>
            </div>
          </div>
        </div>
...
export default {
  ...
  setup() {
    ...
    const activePlans = computed(() => plans.value.filter(plan => plan.isEnabled));
    
    // 计算统计数据
    const totalAlerts = computed(() => {
      return plans.value.reduce((sum, plan) => sum + (plan.alertCount || 0), 0);
    });
    
    const totalData = computed(() => {
      return plans.value.reduce((sum, plan) => sum + (plan.dataCount || 0), 0);
    });
    ...
    const isFormValid = computed(() => {
      return formData.value.keyword.trim() !== '' && formData.value.keywords.length > 0;
    });
    
    // 监听currentPlan变化，更新表单数据
    watch(currentPlan, (newValue) => {
      if (newValue && formMode.value === 'edit') {
        formData.value = {
          keyword: newValue.keyword,
          keywords: Array.isArray(newValue.keywords) ? [...newValue.keywords] : [],
          isEnabled: newValue.isEnabled,
          keyid: newValue.keyid
        };
      } else {
        resetForm();
      }
    ...
    const savePlan = async () => {
      ...
      try {
        const success = await store.dispatch('savePlan', { ...formData.value });
        if (success) {
